...
async function fetchLatestPatchNotes() {
  try {
    ...
    const imageSrc = $details(
      "#patch-notes-container > div:nth-of-type(2) > div > div > span > a"
    )
      .find("img")
      .attr("src");
    ...
    return {
      title: firstPatchElement
        .find("a")
        .text()
        .trim()
        .match(/\d+\.\d+/)[0], // Extract just the version number
      link: fullPatchLink,
      image: imageSrc
        ? imageSrc.startsWith("http")
          ? imageSrc
          : `https:${imageSrc}`
        : null, // Ensure the image URL is complete
    };
  } catch (error) {
